...
strict-peer-dependencies 解决的是 “安装阶段提前拦截”。默认情况下 pnpm 遇到peerDependencies版本不匹配只是警告，不会阻断安装。
...
bridgeHelper 封装的是控制基座UI层面的方法。bridgeHelper 就是基座初始化时把一组方法挂到 window.bridgeHelper 这一命名空间下，子应用在 mount 阶段直接通过 window.bridgeHelper 调用。
